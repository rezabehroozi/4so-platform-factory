...
def validate_source_progress(name: str, component: dict, transition: dict, admission_rows: dict, *, expected_runtime_status: str):
    ...
    row = admission_rows.get(name)
    if resolved:
        if row is not None:
            raise RuntimeError(f"RUNTIME_DEPENDENCY_{name.upper().replace('-','_')}_RESOLVED_ADMISSION_NOT_RETIRED")
        if transition.get("sourceStatus") != "source-acquired":
            raise RuntimeError(f"RUNTIME_DEPENDENCY_{name.upper().replace('-','_')}_SOURCE_STATUS_INVALID")
        return
    if transition.get("sourceStatus") != "pending-byte-acquisition" or not isinstance(row, dict):
        raise RuntimeError(f"RUNTIME_DEPENDENCY_{name.upper().replace('-','_')}_SOURCE_STATUS_INVALID")
    if row.get("selectedVersion") != transition.get("targetRelease") or row.get("status") != "ready-for-acquisition" or row.get("runtimeStatus") != expected_runtime_status:
        raise RuntimeError(f"RUNTIME_DEPENDENCY_{name.upper().replace('-','_')}_DRIFT")
...
def validate(root: Path):
    ...
    runtime_registry = load_json(root / "catalog" / "component-runtime-certification.json")
    holds = {r["component"]: r for r in (runtime_registry.get("spec") or {}).get("runtimeSuitabilityHolds") or []}
    ...
    if {a.get("name") for a in assets} != {"standard-install.yaml","experimental-install.yaml"}:
    ...
    if kg.get("targetRelease") != kgateway.get("release") or kg.get("gatewayApiCompatibility") != "1.4-1.6":
