...
def published_render(base,org):
    ...
    selected=[c for c in components if c.get('metadata',{}).get('name') in ('secure-namespace-foundation','gateway-api')]
    assert len(selected)==2 and all(c['spec']['source']['resolved'] is True for c in selected),selected
...
def main():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        try:
            ...
            # Negative control: an intentionally wrong token for this exact run must be rejected.
            stale_obs_install_result={'taskFenceToken':obs_install['taskFenceToken']+1,'phase':'INSTALL','success':True,'inventoryDigest':obs_install['inventoryDigest'],'renderedDigest':obs_install['renderedDigest'],'checks':[{'key':'fresh-install-target','status':'PASS'}]+[{'key':f'apply/{i}','status':'PASS'} for i in range(1,7)]}
            st,stale_fence,_=req(base+f"/agent/v1/clusters/{cluster_id}/runtime-certification-tasks/{obs_install['runId']}/result",'POST',stale_obs_install_result,{'Authorization':'Bearer '+token,'If-Match':f'"{obs_install["runRevision"]}"'}); assert st==409,(st,stale_fence)
            obs_install_result=dict(stale_obs_install_result); obs_install_result['taskFenceToken']=obs_install['taskFenceToken']
            ...
            print('RUNTIME_CERTIFICATION_FENCE_NEGATIVE_CONTROL_PASS')
            ...
            # COMPONENT_RUNTIME_V1 is a distinct source-bound lifecycle executor.
            component_inventory=inventory('1.18',target=True)
            component_inventory['capabilities'].append('cert.component-runtime')
            st,_,_=req(base+f"/agent/v1/clusters/{cluster_id}/inventory",'POST',component_inventory,{'Authorization':'Bearer '+token}); assert st==200
            st,component_created,_=req(base+'/api/v1/runtime-certifications','POST',{'projectId':project_id,'clusterId':cluster_id,'catalogReleaseId':release_id,'profile':'COMPONENT_RUNTIME_V1','componentName':'gateway-api','namespace':'4so-component-cert'},{'Idempotency-Key':'runtime-cert-component-gateway-1'}); assert st==201 and component_created['run']['state']=='QUEUED' and component_created['run']['componentName']=='gateway-api' and component_created['run']['componentRelease']=='1.5.1',(st,component_created)
            st,component_install,_=req(base+f"/agent/v1/clusters/{cluster_id}/runtime-certification-tasks/next",headers={'Authorization':'Bearer '+token}); assert st==200 and component_install['profile']=='COMPONENT_RUNTIME_V1' and component_install['componentName']=='gateway-api' and len(component_install['resources'])==10,(st,component_install)
            component_install_checks=[{'key':'fresh-install-target','status':'PASS'}]+[{'key':f'apply/{i}','status':'PASS'} for i in range(1,11)]+[{'key':'component-failure-control/duplicate-create-conflict','status':'PASS'}]
            st,component_cp,_=req(base+f"/agent/v1/clusters/{cluster_id}/runtime-certification-tasks/{component_install['runId']}/result",'POST',{'taskFenceToken':component_install['taskFenceToken'],'phase':'INSTALL','success':True,'inventoryDigest':component_install['inventoryDigest'],'renderedDigest':component_install['renderedDigest'],'checks':component_install_checks},{'Authorization':'Bearer '+token,'If-Match':f'"{component_install["runRevision"]}"'}); assert st==200 and component_cp['state']=='VERIFYING' and component_cp['phase']=='VERIFY',(st,component_cp)
            st,component_verify,_=req(base+f"/agent/v1/clusters/{cluster_id}/runtime-certification-tasks/next",headers={'Authorization':'Bearer '+token}); assert st==200 and component_verify['phase']=='VERIFY',(st,component_verify)
            component_checks=[{'key':'durable-install-checkpoint','status':'PASS'}]+[{'key':f'verify/{i}','status':'PASS'} for i in range(1,11)]+[{'key':f'component-readiness/{i}','status':'PASS'} for i in range(1,11)]
            for key in ['nodes-ready','cluster-dns-service','kubernetes-api-tls']:
                component_checks.append({'key':key,'status':'PASS'})
                component_checks.append({'key':'component-dependency/'+key,'status':'PASS'})
            st,bad_component,_=req(base+f"/agent/v1/clusters/{cluster_id}/runtime-certification-tasks/{component_verify['runId']}/result",'POST',{'taskFenceToken':component_verify['taskFenceToken'],'phase':'VERIFY','success':True,'inventoryDigest':component_verify['inventoryDigest'],'renderedDigest':component_verify['renderedDigest'],'checks':[c for c in component_checks if c['key']!='component-readiness/10']},{'Authorization':'Bearer '+token,'If-Match':f'"{component_verify["runRevision"]}"'}); assert st==422 and 'readiness checks' in str(bad_component),(st,bad_component)
            st,component_after_verify,_=req(base+f"/agent/v1/clusters/{cluster_id}/runtime-certification-tasks/{component_verify['runId']}/result",'POST',{'taskFenceToken':component_verify['taskFenceToken'],'phase':'VERIFY','success':True,'inventoryDigest':component_verify['inventoryDigest'],'renderedDigest':component_verify['renderedDigest'],'checks':component_checks},{'Authorization':'Bearer '+token,'If-Match':f'"{component_verify["runRevision"]}"'}); assert st==200 and component_after_verify['state']=='VERIFYING' and component_after_verify['phase']=='FAILURE_RECOVERY',(st,component_after_verify)
            st,component_failure,_=req(base+f"/agent/v1/clusters/{cluster_id}/runtime-certification-tasks/next",headers={'Authorization':'Bearer '+token}); assert st==200 and component_failure['phase']=='FAILURE_RECOVERY',(st,component_failure)
            failure_checks=[{'key':'component-failure-recovery/drift-injected','status':'PASS'},{'key':'component-failure-recovery/reconciled','status':'PASS'},{'key':'component-failure-recovery/readiness-restored','status':'PASS'}]
            st,bad_failure,_=req(base+f"/agent/v1/clusters/{cluster_id}/runtime-certification-tasks/{component_failure['runId']}/result",'POST',{'taskFenceToken':component_failure['taskFenceToken'],'phase':'FAILURE_RECOVERY','success':True,'inventoryDigest':component_failure['inventoryDigest'],'renderedDigest':component_failure['renderedDigest'],'checks':failure_checks[:-1]},{'Authorization':'Bearer '+token,'If-Match':f'"{component_failure["runRevision"]}"'}); assert st==422 and 'readiness-restored' in str(bad_failure),(st,bad_failure)
            st,component_after_failure,_=req(base+f"/agent/v1/clusters/{cluster_id}/runtime-certification-tasks/{component_failure['runId']}/result",'POST',{'taskFenceToken':component_failure['taskFenceToken'],'phase':'FAILURE_RECOVERY','success':True,'inventoryDigest':component_failure['inventoryDigest'],'renderedDigest':component_failure['renderedDigest'],'checks':failure_checks},{'Authorization':'Bearer '+token,'If-Match':f'"{component_failure["runRevision"]}"'}); assert st==200 and component_after_failure['state']=='VERIFYING' and component_after_failure['phase']=='REMOVE',(st,component_after_failure)
            st,component_remove,_=req(base+f"/agent/v1/clusters/{cluster_id}/runtime-certification-tasks/next",headers={'Authorization':'Bearer '+token}); assert st==200 and component_remove['phase']=='REMOVE',(st,component_remove)
            remove_checks=[{'key':f'component-remove/{i}','status':'PASS'} for i in range(1,11)]
            st,component_done,_=req(base+f"/agent/v1/clusters/{cluster_id}/runtime-certification-tasks/{component_remove['runId']}/result",'POST',{'taskFenceToken':component_remove['taskFenceToken'],'phase':'REMOVE','success':True,'inventoryDigest':component_remove['inventoryDigest'],'renderedDigest':component_remove['renderedDigest'],'checks':remove_checks},{'Authorization':'Bearer '+token,'If-Match':f'"{component_remove["runRevision"]}"'}); assert st==200 and component_done['state']=='SUCCEEDED' and component_done['phase']=='REMOVE',(st,component_done)
            st,component_report,_=req(base+f"/api/v1/runtime-certifications/{component_done['id']}/report"); assert st==200 and component_report['claims']['componentName']=='gateway-api' and component_report['claims']['lifecycleStagesCertified']==['install','readiness','dependency','failure','remove'] and component_report['claims']['fullLifecycleCertified'] is False,(st,component_report)
            print('COMPONENT_RUNTIME_FAILURE_REMOVE_PARTIAL_SMOKE_PASS',component_done['id'],component_done['evidenceDigest'])
