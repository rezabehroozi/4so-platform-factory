...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        files={'install.sh':b'#!/bin/sh\nexit 0\n','rke2.tar.gz':b'rke2','rke2-images.tar.zst':b'rke2-images','workloads.oci.tar':(artifacts/'workloads.oci.tar').read_bytes(),'argocd-install.yaml':workload_manifest('argocd',refs['registry.local/argocd']),'argocd-ha-install.yaml':workload_manifest('argocd-ha',refs['registry.local/argocd']),'cnpg-install.yaml':workload_manifest('cnpg',refs['registry.local/cnpg']),'storage-install.yaml':workload_manifest('storage',refs['registry.local/storage'])}
