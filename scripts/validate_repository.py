...
def validate_product_api_contract(root: Path, errors: list[tuple[str,str]], expected_route_count: int) -> int:
    ...
    for marker in ('Code generated by scripts/generate_product_api_contract.py; DO NOT EDIT.',f'ProductAPIContractAuthority = "PRODUCT_API_CONTRACT_AUTHORITY_V1"',f'ProductAPIContractDigest = "{digest}"',f'ProductAPIRouteCount = {len(rows)}'):
        if marker not in go:
...
def main() -> int:
    ...
    if not handoff_plan_path.is_file() or not handoff_script.is_file():
        ...
        if len((hs.get('managementWorkloads') or {}).get('externalImages') or []) != 4 or len((hs.get('managementWorkloads') or {}).get('manifestImageResolution') or []) != 3:
