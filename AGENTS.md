...
- The deterministic Lab runner owns test execution and PASS/FAIL. AI and MCP are advisory/read-only authority surfaces and can never infer Physical PASS; any AI/agent repair changes the source/artifact identity and requires certification to restart from M00 on the new exact SHA.
