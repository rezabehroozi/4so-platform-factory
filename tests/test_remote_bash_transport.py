...
SPEC = importlib.util.spec_from_file_location("remote_bash_transport", ROOT / "scripts" / "run_remote_bash.py")
...
class RemoteBashTransportTests(unittest.TestCase):
    def test_normalizes_bom_crlf_and_bare_cr(self) -> None:
        self.assertEqual(
            module.canonical_bash_payload(b"\\xef\\xbb\\xbfset -eu\\r\\necho one\\recho two"),
            b"set -eu\\necho one\\necho two\\n",
        )
    ...
    def test_builds_strict_argument_safe_ssh_command(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            identity, ssh = root / "id_ed25519", root / "ssh"
            identity.write_text("key", encoding="utf-8")
            ssh.write_text("#!/bin/sh\\n", encoding="utf-8")
            command = module.ssh_command(
                ssh=str(ssh), identity=identity, user="root", host="213.176.28.136",
                connect_timeout=8, connection_attempts=3, alive_interval=5, alive_count=3,
            )
    ...
    def test_rejects_host_metacharacters_before_identity_lookup(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with self.assertRaisesRegex(ValueError, "REMOTE_BASH_HOST_INVALID"):
                module.ssh_command(
                    ssh=str(ssh), identity=root / "missing", user="root", host="host;touch-pwned",
                    connect_timeout=8, connection_attempts=3, alive_interval=5, alive_count=3,
                )
