...
class SupplyChainHandoffTests(unittest.TestCase):
    def test_plan_derives_current_s1_truth_without_promotion(self):
        ...
        transition = spec["runtimeDependencyTransition"]
        self.assertEqual("RUNTIME_DEPENDENCY_TRANSITION_READINESS_V1", transition["readinessAuthority"])
        self.assertTrue(transition["sourceClosureReady"])
        self.assertTrue(transition["transitionExecutionReady"])
        self.assertEqual("runtime-certification-partial", transition["status"])
        self.assertTrue(transition["singleNodeRKE2Certified"])
        self.assertFalse(transition["productTopologyHACertified"])
        self.assertEqual("RKE2_NETWORK_RUNTIME_CERTIFICATION_V1", transition["runtimeEvidence"]["authority"])
        self.assertFalse(transition["currentGatewayApiMutationPerformed"])
