...
class ManagementWorkloadBatchTests(unittest.TestCase):
    ...
    def test_diagnose_reports_current_missing_authority_without_fake_readiness(self):
        ...
        self.assertEqual([], result["pending"]["baseImages"])
        self.assertEqual([], result["pending"]["productImages"])
        ...
        product_receipt = json.loads((ROOT / "lab" / "management-workload-product-image-receipt.json").read_text())
        self.assertEqual(product_receipt["sourceRunId"], result["productReceipt"]["sourceRunId"])
        self.assertEqual(product_receipt["sourceCommitSHA"], result["productReceipt"]["sourceCommitSHA"])
        self.assertTrue(result["productReceipt"]["runtimeRealismVerified"])
