...
| LAB-BUG-004 | P1 | Upgrade/replay | SOURCE FIXED, CI closure pending | `argocd-observer-token` is now written into authoritative foundation desired state before the live Secret patch. Existing valid live tokens reconcile into desired state without rotation, preservation/idempotency/ambiguity negative controls exist, and both HA inline metadata plus non-HA multiline metadata are supported. Runtime evidence waits for Lab access. |
...
| LAB-BUG-011 | P1 | Recovery | AUDITED / OPEN HARDENING | Current legacy DB migration deliberately remains fail-closed after quiesce; automatic restart before ownership outcome is proven would be unsafe. Add durable phase/evidence semantics so resume can distinguish pre-mutation quiesce, ownership transaction completion and desired-workload reconciliation, while preserving fail-closed behavior. |
...
| LAB-BUG-013 | P0 | Credential lifecycle | SOURCE FIXED, CI closure pending | Successful install now persists `INSTALLER_BOOTSTRAP_CREDENTIAL_REVOKED_V1`; a second Start and `prepareHost` fail closed until explicit journaled Reset clears the tombstone. Installer smoke is being updated to use exact source-bound Reset semantics between installation profiles. |
