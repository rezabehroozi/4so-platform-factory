...
- OpenChoreo executor-preparation, mirror, seal and executor-image build inputs now preserve the original filesystem path through regular-file/directory admission instead of calling `resolve()` first. Direct symlink evidence/build inputs therefore fail closed before archive parsing, registry mutation or build execution; focused Python negative controls cover each consumer.
...
- Closure-path code checkpoints in this wave: `90ad9f1a177dcfa50cffa4025669a57e5cd1c6dc` (OpenChoreo seal identity), `27941cebef1cce47b208d492520cb5bb6f93f051` (reviewed S2 predecessor + negative controls, Repository Integrity success), and `4b1727df3619748535142639d2917e01822f7e47` (OpenChoreo evidence-input symlink negative controls; full Repository Integrity tracked independently).

## S1/S2 streaming evidence handoff hardening — 2026-09-24

- The authoritative main advanced during this wave; development rebased to the newer repository state rather than replaying the prior checkpoint.
- Repository Integrity failure on `042e2572fd66740eb16029d6598d48495bff253c` was traced to two derived/test drifts after runtime-suitability persistence: the committed supply-chain handoff lacked the new runtime-hold evidence fields, and one negative control expected the obsolete Cilium error code. Go tests, repository validation, PostgreSQL integration and both provider jobs were already green on that failed run; the Python drift was corrected on main.
- `SUPPLY_CHAIN_HANDOFF_V1` no longer treats any arbitrary historical source lock as an admitted S2 pair. `pair-present` and historical-source blocker retirement now require the exact predecessor reviewed by `COMPONENT_UPGRADE_SOURCE_ADMISSION_V1` for the exact target release. Focused negative controls prove an unreviewed older lock cannot satisfy S2.
- The S1 management archive blocker no longer depends on the mere existence of external/product/manifest requirement rows. `MANAGEMENT_WORKLOAD_OCI_ARCHIVE_PENDING` is derived from `LAB_APPLIANCE_BUNDLE_ACQUISITION_LOCK_V8` and can retire only when `management-workload-oci-archive` is explicitly resolved for the exact release. `MANAGEMENT_IMAGE_DIGEST_LOCKS_PENDING` remains separately tied to unresolved base/core/manifest image-plan rows.
- Current derived handoff truth: component current-source acquisition `17 ready / 0 review-blocked / 3 source-locked`; runtime suitability holds remain `cilium`, `kyverno`, `metallb`; historical acquisition is `2 tagged-source ready / 0 helm-ready / 17 waiting current source / 0 historical locks`; upgrade pair state is `0 pair-present / 2 target-only / 17 target-source-pending / 1 install-only first product release`.
- Management archive authority remains `pending` with acquisition lock status `incomplete`; no S1/S2 runtime or Physical closure is inferred. Open blockers remain `COMPONENT_HISTORICAL_SOURCE_ACQUISITION_PENDING`, `COMPONENT_RUNTIME_UPGRADE_MATRIX_PENDING`, `COMPONENT_SOURCE_ACQUISITION_PENDING`, `MANAGEMENT_IMAGE_DIGEST_LOCKS_PENDING`, `MANAGEMENT_WORKLOAD_OCI_ARCHIVE_PENDING`, `SOURCE_LOCKS_PENDING`, and `UPSTREAM_RUNTIME_SUITABILITY_HOLDS_PENDING`.
- Final source checkpoint for this handoff hardening wave: `a1f040a407e4c31c7020cb89056652d8a7bdac81`; Repository Integrity run `36046717020` completed **SUCCESS**, including repository validation, Go build/tests/vet, Python tests, Autopilot self-test, installer isolated smoke, explicit clean-clone verification, PostgreSQL behavioral integration, Terraform provider and Crossplane provider. The handoff validator now reports the first exact structural drift path instead of a generic snapshot mismatch, and the committed runtime-hold snapshot is aligned with the canonical derived shape.

## S1/S2 acquisition execution + J8 output-admission checkpoint — 2026-09-24

- Test-first execution-path audit reproduced four real acquisition defects before production fixes: staged S1 replay re-entered already-installed components, S2 historical resume treated a symlink source-lock as already installed, and direct Helm/tagged installs mutated canonical source state before repository validation without first refreshing the derived supply-chain handoff.
- S1 staged install is now restart-safe: an entry is skipped only when the exact current source-lock already binds the staged component/version/upstream URL/upstream artifact digest. A mismatched installed lock fails closed rather than silently skipping or replaying the immutable import.
- S2 historical resume now uses one exact no-follow source-lock admission for both queue classification and staged installation. Direct or broken symlinks fail closed; a regular lock must bind the reviewed component, predecessor version and upstream URL before it can count as already acquired.
- Direct current Helm and historical Helm/tagged acquisition now refresh `lab/supply-chain-handoff-plan.json` immediately after the canonical catalog mutation and before `validate_repository.py`. Connected `--execute` and staged/offline install therefore share the same derived-state convergence boundary.
- J8 external-byte producers now share `admit_output_path` from the canonical OpenChoreo runtime contract. Acquisition ZIP, executor context, zot mirror evidence, BuildKit executor evidence and final runtime-source seal preserve the caller path until direct-symlink admission and no longer erase output symlink identity via an early `resolve()`.
- RED evidence was Repository Integrity run `36056302662` for S1/S2/direct-install regressions and run `36057112133` for J8 output admission. The final code checkpoint `ecc53ef4ba1b631dd7a0deafa011ba615810c3e7` is proven by Repository Integrity run `36057445380` = **SUCCESS**, including repository validation, Go build/tests/vet, Python suite, Autopilot self-test, installer smoke, explicit clean-clone verification, PostgreSQL behavioral integration, Terraform provider and Crossplane provider.
- Supply-chain truth is unchanged because this wave produced no external bytes: current component source state remains `17 ready / 0 review / 3 locked`; historical state remains `2 tagged ready / 0 helm ready / 17 waiting current / 0 historical locked`; upgrade pairs remain `0 pair-present / 2 target-only / 17 target-source-pending / 1 install-only`. No Runtime or Physical PASS is inferred.

## Exact component source closure + Gateway API dependency acquisition checkpoint — 2026-09-25

- S1 catalog component source acquisition is now byte-complete at the source-lock layer: all 20 catalog components have exact current source locks and the canonical current acquisition queue is empty (`20 locked / 0 ready / 0 review-blocked`).
- S2 historical-source acquisition is also byte-complete for every upgrade-applicable component: `19/19` reviewed predecessors have exact historical source locks; the upgrade-pair authority reports `19 pair-present / 0 target-source-pending / 0 target-only`, while `secure-namespace-foundation` remains the intentional first-product-release install-only case. This is source-pair evidence only; no runtime upgrade PASS is inferred.
- The remaining runtime suitability holds are independent of source acquisition. Cilium 1.20.1, Kyverno 3.8.2 and MetalLB 0.16.1 keep their runtime hold semantics even though their exact current source bytes are acquired.
- The Cilium dependency transition advanced one real external-byte boundary: both official Gateway API v1.6.1 release assets were fetched on GitHub Actions, exact-size/SHA-256 verified, committed under `catalog/runtime-dependencies/gateway-api/1.6.1/`, embedded into the product binary authority and independently revalidated by Python and Go. `catalog/runtime-dependency-transition.json` is therefore `runtime-certification-pending`, not complete.
- Gateway API v1.6.1 standard-install evidence: 1,170,953 bytes, SHA-256 `24d931f22abd8e40c973264319ead7cfa09d0fb7716b7ab1ee2ff174cb063a73`. Experimental-install evidence: 1,402,117 bytes, SHA-256 `d7fa77650e4ef28fca0411536fcb5e237deb4d50301cfded3be49d9a1b7bbd02`.
- Acquisition run `36114468229` completed **SUCCESS** and produced canonical byte commit `f7b10c9fbbd4b7d79d7afca4fe9e80314bacbc5c`. Subsequent source hardening embeds and re-verifies those bytes; this checkpoint does not claim cluster mutation, runtime certification or Physical PASS.
...
- The dominant remaining S1 supply-chain work is now management base/product image closure plus final OCI byte assembly. Exact Argo CD normal/HA, CloudNativePG and Longhorn manifest image identities are resolved and sealed by canonical receipt evidence; `MANAGEMENT_IMAGE_DIGEST_LOCKS_PENDING` remains truthful because base/product images are still open, and `MANAGEMENT_WORKLOAD_OCI_ARCHIVE_PENDING` remains truthful until the final archive is actually assembled.
...
- This evidence does **not** close `MANAGEMENT_WORKLOAD_OCI_ARCHIVE_PENDING`. Since this checkpoint, exact manifest image resolution has been completed separately, but `lab/appliance-bundle-acquisition-lock.json` remains `incomplete` because the canonical full management-workload OCI archive still requires current exact base/product image evidence and final byte assembly. The V5 image plan remains intent/input authority; effective manifest readiness is derived from its exact plan-bound receipt rather than mutating pending plan rows.
...
## Management manifest image evidence closure — 2026-09-25

- GitHub Actions run `36132417496` completed **SUCCESS** and produced canonical `MANAGEMENT_WORKLOAD_MANIFEST_IMAGE_RECEIPT_V1` evidence for exact release `0.0.363`, covering all four V5 plan authorities: Argo CD normal, Argo CD HA, CloudNativePG and Longhorn.
- The receipt remains deliberately scoped: `resolved=true`, `runtimeCertified=false`, `physicalCertified=false`. It does not claim image-byte presence in the final archive and does not promote Runtime or Physical PASS.
- Owner-layer drift found after the receipt was fixed on main: the plan schema now admits the four-set HA shape; a fail-closed receipt verifier binds plan digest, source/resolved paths, file SHA-256, resolution-lock authority and every exact image reference; supply-chain handoff/diagnostics derive manifest readiness from that receipt rather than stale plan-row state.
- Negative control now rejects post-receipt resolved-manifest byte tampering, and Repository Integrity directly validates the canonical manifest receipt.
- Exact source checkpoint `a9700b085d939a2aef6f93925d47af3159782568` passed Repository Integrity run `36136883680` with clean-clone, Go/Python, Autopilot, installer smoke, PostgreSQL behavioral integration, Terraform provider and Crossplane provider all green.
- Remaining management image debt after this closure is current exact base/product image evidence (especially maintenance-toolchain) plus acquisition/merge of resolved manifest image bytes and final OCI assembly. `lab/appliance-bundle-acquisition-lock.json` therefore correctly remains `incomplete`.
...
J8 owns implementation closure for the remaining adaptation work. Application-platform persistence and Product API/SDK/MCP convergence are present through migration `0082`; Fleet session authority is durable through migration `0083`; Delivery Insights preserves UNKNOWN when deployment evidence is missing; and OpenChoreo assessment remains fail-closed until exact source and durable lifecycle prerequisites exist. Fleet transport-runtime execution and deployment/release evidence ingestion are source-implemented; the remaining owner work is the exact-source OpenChoreo lifecycle adapter. J8 is Expansion scope and does **not** gate Core C9 Feature Freeze.


## Current handoff checkpoint — 2026-09-25 exact upstream acquisition closure

- Canonical source checkpoint before this handoff update: `ba4b1da2b4d33c1cfaaea3a83f2894d25ef04600`. The last fully completed pre-acquisition repository-integrity run on exact green base `e0edbede575bad5fe2aa1c74792271bea0656125` was run `36109368449` and passed Repository Validator, Go, Python, Autopilot recovery self-test, installer smoke, explicit clean-clone verification, PostgreSQL behavioral integration, Terraform provider and Crossplane provider.
- S1 exact current-source acquisition is closed at **20/20** catalog components. `lab/supply-chain-handoff-plan.json` now reports `alreadySourceLocked=20`, `ready=0`, `reviewBlocked=0`.
- S2 reviewed historical-source acquisition is closed for every upgrade-eligible component: **19/19 pair-present**, with `secure-namespace-foundation` correctly remaining the single install-only first product release. Historical queue state is `alreadyHistoricalSourceLocked=19`, `helmReady=0`, `taggedSourceSetReady=0`, `waitingCurrentSource=0`, and there are **0 pending upgrade pairs**.
- The final current-source wave acquired Capsule 0.13.11, Loki 18.12.1 and VictoriaMetrics k8s-stack 0.90.2 after owner-layer fixes for canonical render-value binding and canonical-render-only image inventory. Their reviewed predecessors 0.13.10, 18.12.0 and 0.90.1 were then acquired through the same exact historical authority.
- Runtime suitability remains deliberately independent of source closure. Cilium 1.20.1 remains held by `RUNTIME_DEPENDENCY_TRANSITION_V1` until the exact Gateway API 1.6.1 transition is acquired/certified; Kyverno 3.8.2 and MetalLB 0.16.1 remain explicit runtime-review holds. Source acquisition does **not** imply runtime, production or Physical PASS.
- J8 exact OpenChoreo acquisition now has canonical Git evidence in `lab/openchoreo-runtime-acquisition-receipt.json`: OpenChoreo `1.3.0`, upstream commit `178dfbde3e3343e5ac151b88a2f203f523f97480`, source archive SHA-256 `46077843fab307220057b7888372281acc00138740f194d6fd6c1fa6bd68f517`, exact acquisition bundle SHA-256 `1a7972c3511abbe573944d59a2eb0d7dfff9da0e7a4442840fd8941390ca2a3e`, 2 rendered planes and 6 digest-pinned image references.
- The persisted J8 receipt intentionally states `executionReady=false` and `imageBytesIncluded=false`. It proves exact source/chart/render/image identity only. zot mirroring, executor image build, durable lifecycle execution and runtime certification remain separate gates.
- Acquisition wave 3 GitHub Actions run `36109704823` completed **SUCCESS**, including S1/S2 acquisition, durable push, OpenChoreo acquisition, artifact upload and final honest failure gate. Its retained workflow artifact `openchoreo-v1.3.0-exact-acquisition` has archive digest `sha256:89d5961e27e8e293070ba5de02c5d400e4fe48fcdcb40db4e50adc6d0e5ba411`.
- The temporary `.github/workflows/closure-acquisition-once.yml` workflow was removed after closure to avoid permanent meta-artifact sprawl.

### Next owner-layer work

Do not reopen S1/S2 source acquisition unless an explicit version/source migration is approved. Continue from runtime/execution closure: (1) complete the Gateway API 1.6.1 dependency transition needed by Cilium; (2) disposition Kyverno/MetalLB runtime holds with real evidence; (3) mirror the exact J8 image set into canonical zot authority, build the product-owned OpenChoreo executor image through BuildKit, seal the runtime source, and exercise durable lifecycle/readback without inferring rollback or runtime PASS; (4) advance component runtime upgrade execution matrices from source-pair admission to real upgrade/readiness/failure evidence. Exact-SHA Physical Runtime remains NOT RUN unless the matching SHA is physically exercised.


## Current handoff checkpoint — 2026-09-25 management external evidence closure

- Canonical source checkpoint before this documentation seal: `311003a436fa28c2dc5a877e1d729764a7e506db`.
- S1 current-source acquisition remains **20/20 exact source-locked**. S2 reviewed predecessor acquisition remains **19/19 pair-present**, with `secure-namespace-foundation` as the single install-only first product release. These are source/evidence closures only and do not imply component runtime certification.
- Gateway API runtime-dependency external bytes are acquired and verified for exact `v1.6.1`; `RUNTIME_DEPENDENCY_TRANSITION_V1` is now `runtime-certification-pending`, not acquisition-pending. Cilium runtime suitability therefore remains open until real runtime transition/certification evidence exists.
- Management external image acquisition workflow run `36115673607` completed **SUCCESS** on exact source SHA `e9b20f8334e270e792d58367f93535d11861b196`. The run built and verified the exact release with admitted Go `1.27.1 linux/amd64`, acquired and offline-verified Forgejo, Keycloak, PostgreSQL and Zot, sealed a Git-safe receipt, assembled an external-only OCI archive, and uploaded both release/receipt and OCI-byte artifacts.
- Canonical Git now persists that signed receipt at `lab/management-workload-external-image-receipt.json` with authority `MANAGEMENT_WORKLOAD_EXTERNAL_IMAGE_RECEIPT_V1`, `offlineVerified=true`, `runtimeCertified=false`, `physicalCertified=false`, release artifact digest `sha256:5afaaf777ffaafb2e4e1d372fc78e42b28f697ba531836728207df1d558ae850`, and source run `36115673607`.
- Exact external management image identities from the receipt: Forgejo `sha256:b0d569cd2e55b092a9302797e495249e981e6ce86f7001a237e7bbdc3f0c3a3f`; Keycloak `sha256:e03b95891c214fba3ab72f12f40c983211549e817c5fce8d0f37761db04ae185`; PostgreSQL `sha256:91eb910c44c7ed13f7f1a4ccadaa9ca72ef14cddc04cacb6e070e48eb44731a3`; Zot `sha256:95a837a0afacf5b7edc0c92493f04beee6891989b8d2fd50a00cf65a1e6d4fd5`.
- The same run assembled external-only OCI bytes: `578164736` bytes, `sha256:a8dd51e5131615b7f3bf78602a85bcbe317c68d0c5f2cfae4ff69be84e4ea09b`, 52 blobs, 4 images. Retained GitHub artifact `management-external-images-oci` is artifact ID `10854952256`, uploaded ZIP digest `sha256:7441bf1365e7e88c4c014e7345f2993424b9970fbc116be8dae674cb9f9cbd3f`.
- Retained exact-release evidence artifact is ID `10854772340`, uploaded ZIP digest `sha256:3359d244a76333716e1fa01c03e3288baffba6735dc083b7d5986cd3e912ac05`.
- Repository Integrity run `36115673546` for exact SHA `e9b20f8334e270e792d58367f93535d11861b196` completed **SUCCESS**. The evidence receipt persistence commit after that run is intentionally documentation/evidence-only and must receive its own exact-SHA Repository Integrity readback before final handoff.
- `lab/appliance-bundle-acquisition-lock.json` remains `incomplete`; its only missing authority is still `management-workload-oci-archive`. The external-only OCI archive above is real evidence but is **not** the final product-owned management workload OCI archive because product images and manifest-derived image sets are not fully closed.
- `lab/management-workload-image-build-plan.json` intentionally retains pending input rows, but exact manifest readiness is now derived from `MANAGEMENT_WORKLOAD_MANIFEST_IMAGE_RECEIPT_V1`: all four planned manifest sets (Argo CD normal + HA, CloudNativePG, Longhorn) are plan-bound, digest-pinned and byte-revalidated. Base/product image evidence is still open, so do not mark `MANAGEMENT_IMAGE_DIGEST_LOCKS_PENDING` or `MANAGEMENT_WORKLOAD_OCI_ARCHIVE_PENDING` closed.
- Remaining high-value closure lanes: (1) exact product management images and base-image compatibility/digests, including maintenance toolchain; (2) acquire/merge the already-resolved manifest image bytes into final assembly; (3) final product-owned management workload OCI archive and sealed ApplianceBundle; (4) Cilium/Gateway runtime certification plus Kyverno/MetalLB runtime suitability disposition; (5) J8 exact image-byte zot mirror, BuildKit executor image, sealed runtime source and durable lifecycle evidence; (6) C7W external named-client interoperability; (7) H1/I1 managed/disconnected OKD runtime evidence; (8) final Exact-SHA Physical Runtime certification.
- Remote branch `tmp/j1-terraform-provider` remains stale and has no unique changes; delete it when a ref-deletion-capable Git tool is available. It is not a development blocker.
- Exact-SHA Physical Runtime remains **NOT RUN** for the current main SHA. No physical or production PASS is inferred from source, CI, artifact, image or OCI evidence.

### Next chat execution checkpoint

Refresh `origin/main` first and trust the newer HEAD if main advanced. Read this file plus `internal/targetmodel/program.go`, `lab/supply-chain-handoff-plan.json`, `lab/appliance-bundle-acquisition-lock.json`, `lab/management-workload-image-build-plan.json`, `lab/management-workload-external-image-receipt.json`, `catalog/runtime-dependency-transition.json`, and the J8 OpenChoreo receipt/runtime-source authorities. Confirm Repository Integrity for the exact newest main SHA before mutation. Continue in large owner-layer waves: first import/consume the verified management external receipt without inflating archive readiness, then close product image/base-image and manifest-image digest evidence, assemble the final product-owned management workload OCI archive and retire `MANAGEMENT_WORKLOAD_OCI_ARCHIVE_PENDING`; in parallel advance Gateway/Cilium runtime certification, Kyverno/MetalLB runtime suitability, J8 zot/BuildKit/seal/lifecycle evidence, C7W external clients, and H1/I1 runtime evidence. Do not reopen S1/S2 source acquisition unless an explicit version/source migration is approved. Keep Physical PASS separate and exact-SHA-only.


## 2026-09-26 — Management OCI + J8 runtime-realism closure wave

- Final management workload OCI assembly is now proven by GitHub Actions run `36222903265` = **SUCCESS**. The exact archive contains **20 images / 238 blobs**, is **2,306,048,512 bytes**, and is sealed by `MANAGEMENT_WORKLOAD_OCI_ARCHIVE_RECEIPT_V1` at `lab/management-workload-oci-archive-receipt.json`. Archive SHA-256 is `sha256:7b6df110a4c661c5f03ac05499dd1ec15cd2e1956dfa4c50e128d107b0fb7fbe`. Inventory and import-addressability readback passed. This proves exact archive assembly, not public distribution readiness, runtime certification or Physical PASS.
- Manifest-derived management images were regenerated for explicit `linux/amd64` semantics and sealed by run `36221167418` = **SUCCESS** before the final OCI archive was rebuilt. The prior multi-arch/index-vs-platform digest ambiguity is therefore removed from the canonical manifest receipt.
