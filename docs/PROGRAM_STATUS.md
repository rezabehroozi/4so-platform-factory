...
- `lab/management-workload-image-build-plan.json` still has pending base-image/product-image/manifest-image rows. Do not mark `MANAGEMENT_IMAGE_DIGEST_LOCKS_PENDING` or `MANAGEMENT_WORKLOAD_OCI_ARCHIVE_PENDING` closed until those canonical authorities are updated from exact evidence.
- Remaining high-value closure lanes: (1) exact product management images and base-image compatibility/digests; (2) exact manifest image resolution for Argo CD, CNPG and Longhorn manifests; (3) final product-owned management workload OCI archive and sealed ApplianceBundle; (4) Cilium/Gateway runtime certification plus Kyverno/MetalLB runtime suitability disposition; (5) J8 exact image-byte zot mirror, BuildKit executor image, sealed runtime source and durable lifecycle evidence; (6) C7W external named-client interoperability; (7) H1/I1 managed/disconnected OKD runtime evidence; (8) final Exact-SHA Physical Runtime certification.
