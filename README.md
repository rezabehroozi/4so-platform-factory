...
Release 0.0.362 advances executable roadmap authority at `PROGRAM_PHASE_MODEL_V67` and preserves `PROGRAM_PROGRESS_MODEL_V2` at **30/35 (85%)** Core+Expansion software phases source-closed; mandatory Core source/software closure remains **25/25 (100%)** and Core phase/release closure remains **19/25 (76%)**. This hardening release closes Autopilot recovery defects without manufacturing roadmap progress: non-PASS stage outcomes retain their checkpoint, Windows timeout/restart cleanup is bound to PID creation identity and terminates descendant process trees, and live Field Campaign recovery remains fail-closed unless the operator explicitly supplies `PLATFORM_FACTORY_AUTOPILOT_FIELD_RESUME_CONFIRMATION=RESUME`. A FAILED campaign is diagnosed before any permitted resume, while resumed campaigns must still watch to completion, freshly collect exact-release-bound evidence and independently verify it. Linux CI owns the canonical Go/Installer runtime checks because the product source uses Linux no-follow/openat/flock semantics. No Fleet Incident/SLO closure, Physical/Exact-SHA PASS or production readiness is claimed. See `docs/PHASE_STATUS_V67.md`.
