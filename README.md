...
Release 0.0.361 advances executable roadmap authority to `PROGRAM_PHASE_MODEL_V67` with J7 FinOps v2 source closure. `FINOPS_BUDGET_POLICY_AUTHORITY_V1` adds immutable organization/project-scoped PostgreSQL-backed budget policies; `FINOPS_FORECAST_ANOMALY_RIGHTSIZING_AUTHORITY_V1` derives forecast, spend anomaly and review-only rightsizing only from measured FinOps evidence. Missing telemetry/rate coverage remains UNKNOWN rather than zero, rightsizing requires capacity evidence no older than 24 hours and is never auto-applied, and PostgreSQL organization-level budget identity now matches Memory/File uniqueness semantics despite nullable project scope. Generated Product API/Go SDK/MCP parity and Operator Console surfaces cover the new routes. `PROGRAM_PROGRESS_MODEL_V2` now reports **30/35 (85%)** Core+Expansion software phases source-closed; mandatory Core source/software closure remains **25/25 (100%)** and Core phase/release closure remains **19/25 (76%)**. No Fleet Incident/SLO closure, Physical/Exact-SHA PASS or production readiness is claimed. See `docs/PHASE_STATUS_V67.md`.
