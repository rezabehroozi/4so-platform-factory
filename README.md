> **Current development roadmap authority:** `PROGRAM_PHASE_MODEL_V75`. Core Freeze is quality-gated and intentionally narrower than the competitive whole-product software roadmap. `source-implemented` is source-level only and never implies Generated Runtime, Integration, Physical or production PASS. Physical/Exact-SHA evidence gates never block pre-physical software development.

Current `main` uses `PROGRAM_PHASE_MODEL_V75` and `PROGRAM_PROGRESS_MODEL_V2`: mandatory Core source/software closure is **25/25 (100%)**, Core closure/release readiness remains **19/25 (76%)**, and Core+Expansion pre-physical software closure is **36/36 (100%)**. J3 Virtual Cluster is source-implemented with durable executor/lifecycle fencing, bounded diagnostics and measured FinOps attribution. J8 Application Platform Composition now persists and exposes WorkloadType/CapabilityTrait, ManagedResourceType, WorkspaceProfile, immutable ApplicationRelease and revision-fenced EnvironmentBinding authority through PostgreSQL/Product API/SDK/MCP. Fleet gateway persistent transport now includes mTLS WSS, restart-safe durable epoch readback, server-owned gateway instance identity, heartbeat and bounded reconnect/backoff; Delivery Insights consumes durable deployment evidence. J8 is source-implemented end-to-end for the exact-source durable OpenChoreo runtime adapter; exact v1.3.0 source/chart/image acquisition, six-image ephemeral-zot mirror, exact BuildKit executor build/readback and runtime-realism sealing now have durable evidence, while persistent product-zot production sealing remains closure-blocked. V75 preserves `sourceStatus` from `closureStatus`, so external/runtime/physical evidence can remain BLOCKED without turning completed software back into source debt. The current execution wave is `W1-core-closure-blitz`; S1 acquisition and S2 component certification are streamed rather than serialized, while MCP/OKD/external-acquisition evidence can advance independently. I2 Edge/Sovereign is source-implemented with bounded local authority, reconnect review, boot-attestation assessment, disconnected local-AI profile validation and an Operator Console workflow; these source semantics do not claim disconnected runtime or Physical certification. Physical/Exact-SHA certification remains deferred until C9 and is never inferred from source progress. See `docs/PROGRAM_STATUS.md`.
