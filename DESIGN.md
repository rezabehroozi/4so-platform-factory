...
## OpenChoreo v1.3 reference-adoption boundary

`OPENCHOREO_REFERENCE_ADOPTION_V1` is the executable architecture decision for selective OpenChoreo adoption. The reviewed baseline is OpenChoreo v1.3.x at upstream commit `178dfbde3e3343e5ac151b88a2f203f523f97480` (official v1.3.0 tag commit verified 2026-09-24). OpenChoreo is a reference implementation for application-platform abstractions, multi-cluster agent/gateway transport, MCP authorization and AI/operational-insight patterns; it is not another 4SO source of truth.

The 4SO composition chain is **WorkloadType + CapabilityTrait / ManagedResourceType / WorkspaceProfile -> Immutable Release -> Environment Binding -> capability resolution -> rendered runtime -> observed runtime**. The source contracts are now executable as `WORKLOAD_TYPE_AUTHORITY_V1`, `CAPABILITY_TRAIT_AUTHORITY_V1`, `WORKLOAD_COMPOSITION_AUTHORITY_V1`, `MANAGED_RESOURCE_TYPE_AUTHORITY_V1`, `WORKSPACE_PROFILE_AUTHORITY_V1`, `APPLICATION_RELEASE_AUTHORITY_V1`, and `ENVIRONMENT_BINDING_AUTHORITY_V1`; PostgreSQL persistence and Product API convergence remain J8 work. Desired authority remains PostgreSQL. Forgejo remains desired-state history, Argo CD remains reconciliation, and rendered/observed target objects are evidence/executor state rather than control-plane authority. Secret-bearing resource outputs are references only.

Fleet transport hardening may selectively adapt target-initiated persistent transport, mTLS, heartbeat, reconnect/backoff, session epochs and gateway draining, but must preserve existing 4SO enrollment identity, certificate revocation, task leases, dispatch acknowledgements, mutation fences and `RECOVERY_REQUIRED` ambiguity handling.

Cost Insights, Delivery/DORA Insights and audit separation are pattern inputs to existing 4SO FinOps, Incident/SLO and Audit authorities. They remain derived projections; missing telemetry remains missing, and analytics cannot grant release or Physical PASS.
...
`FLEET_AGENT_GATEWAY_SESSION_AUTHORITY_V1` plus migration `0083` owns target-initiated mTLS session identity, heartbeat freshness, stale-session epoch replacement, one-active-session-per-cluster and drain/close dispatch fencing. This is session authority, not yet proof of a production long-lived WSS transport implementation.

`DELIVERY_INSIGHTS_AUTHORITY_V1` is a bounded evidence projection. Missing deployment/source-commit evidence remains `UNKNOWN`; incident recovery can be observed independently. A numeric zero is never synthesized from missing delivery evidence, and the projection explicitly reports `physicalCertificationInferred=false`.

OpenChoreo remains optional and target-only. The assessment endpoint may report eligibility only when discovery, duplicate-stack resolution, exact-source admission, disconnected mirror admission where required, and durable lifecycle readiness are all present; assessment never implies Physical certification.
