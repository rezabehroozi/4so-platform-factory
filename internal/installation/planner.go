...
func NormalizeRequest(request InstallRequest) InstallRequest {
	...
	request.Services.Database = normalizeService(request.Services.Database, "cloudnative-pg")
...
func CreatePlanWithCapabilities(request InstallRequest, capabilities RuntimeCapabilities) (InstallationPlan, error) {
	...
	validateExternal("database", request.Services.Database, &blockers)
	...
	validateServiceProvider("database", request.Services.Database, "cloudnative-pg", []string{"postgresql"}, &blockers)
