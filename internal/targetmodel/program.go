...
const (
	ProgramAuthorityMethod                = "PROGRAM_PHASE_MODEL_V71"
...
func ProgramRoadmapModel() ProgramRoadmap {
	...
	phases := []ProgramPhase{
		...
		{ID: "J8-application-platform-abstraction-composition", Order: 35, Status: ProgramStatusBlocked, DeliveryTier: ProgramTierExpansion, RequiredForFeatureFreeze: false, Objective: "Productize workload/trait, managed-resource, WorkspaceProfile and immutable release-binding composition; harden Fleet session transport; extend evidence-backed delivery insights; and define a disabled-by-default OpenChoreo target adapter without importing a second control plane.", DependsOn: []string{"E-certified-platform-template-workspace-foundation", "J4-product-api-contract-recovery-foundation", "J5-resource-scope-owner-closure"}, ParallelWith: []string{"J3-virtual-cluster-profile", "J6-fleet-reliability-incident-intelligence", "J7-finops-v2-budget-forecast-rightsizing", "I2-edge-sovereign-extension"}, Blockers: []string{"WORKLOAD_TYPE_TRAIT_COMPOSITION_PENDING", "MANAGED_RESOURCE_TYPE_OUTPUT_REFERENCE_PENDING", "WORKSPACE_PROFILE_RELEASE_BINDING_PENDING", "FLEET_AGENT_GATEWAY_SESSION_HARDENING_PENDING", "DELIVERY_INSIGHTS_PROJECTION_PENDING", "OPTIONAL_OPENCHOREO_TARGET_ADAPTER_PENDING"}, Evidence: []string{OpenChoreoReferenceAuthority, "TARGET_ARCHITECTURE_MODEL_V1", "PLATFORM_TEMPLATE_AUTHORITY_V1", "WORKSPACE_AUTHORITY_V1", "TARGET_CAPABILITY_RESOLVER_V1", "MCP_REMOTE_OAUTH_DELEGATION_ARCHITECTURE_V1", "FINOPS_USAGE_MEASUREMENT_AUTHORITY_V1", "INCIDENT_AUTHORITY_V1"}, ExitCriteria: []string{"WorkloadType/CapabilityTrait composition is typed, immutable/versioned and capability-resolved without blueprint variant explosion", "ManagedResourceType outputs and secrets are typed references; target CRDs/operators/Crossplane never become product SoT", "WorkspaceProfile and immutable EnvironmentBinding promotion are explicit and preserve desired/rendered/observed authority separation", "Fleet streaming transport has outbound mTLS identity, heartbeat, reconnect/backoff, session epoch, duplicate-session and gateway-drain semantics without bypassing task fences", "delivery/DORA and AI insights are bounded derived projections over exact releases, operations, incidents, audit and measured telemetry", "optional OpenChoreo target adapter is disabled by default, target-only, exact-source/disconnected-capable and duplicate-stack aware"}},
