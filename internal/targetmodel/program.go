...
const (
	ProgramAuthorityMethod                = "PROGRAM_PHASE_MODEL_V70"
...
func ProgramRoadmapModel() ProgramRoadmap {
	...
	phases := []ProgramPhase{
		...
		{ID: "J3-virtual-cluster-profile", Order: 29, Status: ProgramStatusBlocked, DeliveryTier: ProgramTierExpansion, RequiredForFeatureFreeze: false, Objective: "Develop virtual-cluster workspace isolation, lifecycle, quota, FinOps and developer-mode contracts before physical certification as a separate product lifecycle target.", DependsOn: []string{"E-certified-platform-template-workspace-foundation", "G2-generalized-day2-campaign-engine"}, ParallelWith: []string{"J1-automation-external-integrations", "J2-finops-usage", "I2-edge-sovereign-extension"}, Blockers: []string{"VIRTUAL_CLUSTER_DURABLE_RUNTIME_PENDING"}, Evidence: []string{"VIRTUAL_CLUSTER_PROFILE_AUTHORITY_V1", "VIRTUAL_CLUSTER_LIFECYCLE_AUTHORITY_V1", "VIRTUAL_CLUSTER_DURABLE_AUTHORITY_V1", "internal/virtualcluster", "migrations/0078_virtual_cluster_authority.sql", "WORKSPACE_AUTHORITY_V1", "WorkspaceBinding revision fence", "POST /api/v1/workspaces/{id}/virtual-clusters", "GET /api/v1/workspaces/{id}/virtual-clusters", "MCP_ROUTE_PARITY_AUTHORITY_V1", "POSTGRES_BEHAVIORAL_INTEGRATION_V1", "operator-console:virtual-cluster-desired-state"}, ExitCriteria: []string{"virtual cluster plans derive host cluster/namespace authority only from an ACTIVE same-project WorkspaceBinding", "developer/team profiles enforce bounded quota and exact Kubernetes version admission", "lifecycle mutations are idempotency/fence-bound and ambiguous outcomes enter RECOVERY_REQUIRED without automatic replay", "durable PostgreSQL lifecycle plus REST/MCP/Console parity use the same workspace-bound authority"}},
