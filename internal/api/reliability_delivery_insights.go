...
func parseDeliveryInsightsWindow(r *http.Request) (time.Time,time.Time,error) {
	...
	if !to.After(from){return time.Time{},time.Time{},fmt.Errorf("invalid delivery insight window")};return from.UTC(),to.UTC(),nil
}
func (s *Server) reliabilityDeliveryInsights(w http.ResponseWriter,r *http.Request){
	...
	for _,incident:=range incidents{
		evidence=append(evidence,reliability.DeliveryEvidence{EvidenceID:"incident-opened:"+incident.ID,ProjectID:projectID,EventType:reliability.DeliveryEventIncidentOpened,OccurredAt:incident.CreatedAt,IncidentID:incident.ID})
		if incident.State==reliability.IncidentResolved{evidence=append(evidence,reliability.DeliveryEvidence{EvidenceID:"incident-resolved:"+incident.ID,ProjectID:projectID,EventType:reliability.DeliveryEventIncidentResolved,OccurredAt:incident.UpdatedAt,IncidentID:incident.ID})}
	}
