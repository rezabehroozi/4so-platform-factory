...
type reliabilityIncidentResolveInput struct { ResolutionSummary string `json:"resolutionSummary"` }
...
func expectedReliabilityRevision(r *http.Request) (int64, error) {
	...
	if raw == "" { return 0, controlplane.ErrValidation }
	raw = strings.TrimSpace(strings.Trim(raw, `"`))
	value, err := strconv.ParseInt(raw, 10, 64)
	if err != nil || value <= 0 { return 0, controlplane.ErrValidation }
	return value, nil
}
...
func (s *Server) createReliabilityIncident(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil { writeScopeError(w, err); return }
	input.ClusterID = strings.TrimSpace(input.ClusterID)
	if input.ClusterID != "" {
		cluster, lookupErr := s.store.GetManagedCluster(r.Context(), input.ClusterID)
		if lookupErr != nil || cluster.ProjectID != project.ID { writeStoreError(w, controlplane.ErrNotFound); return }
	}
	store, ok := s.reliabilityStore(); if !ok { writeReliabilityStoreUnavailable(w); return }
	created, err := store.CreateIncident(r.Context(), reliability.Incident{OrganizationID: project.OrganizationID, ProjectID: project.ID, ClusterID: input.ClusterID, Service: strings.TrimSpace(input.Service), Severity: strings.TrimSpace(input.Severity), State: reliability.IncidentOpen}, actor)
...
func (s *Server) getReliabilityIncident(w http.ResponseWriter, r *http.Request) {
	store, ok := s.reliabilityStore(); if !ok { writeReliabilityStoreUnavailable(w); return }
	incident, err := store.GetIncident(r.Context(), r.PathValue("id")); if err != nil { writeStoreError(w, err); return }
	if _, err = s.requireProjectAccess(r, incident.ProjectID, organizationRead); err != nil { writeScopeError(w, err); return }
	writeJSON(w, http.StatusOK, incident)
}

func (s *Server) acknowledgeReliabilityIncident(w http.ResponseWriter, r *http.Request) { s.transitionReliabilityIncident(w, r, reliability.IncidentActionAcknowledge, "") }
...
func (s *Server) transitionReliabilityIncident(w http.ResponseWriter, r *http.Request, action, summary string) {
	...
	store, ok := s.reliabilityStore(); if !ok { writeReliabilityStoreUnavailable(w); return }
	incident, err := store.GetIncident(r.Context(), r.PathValue("id")); if err != nil { writeStoreError(w, err); return }
	if _, err = s.requireProjectAccess(r, incident.ProjectID, organizationWrite); err != nil { writeScopeError(w, err); return }
	expected, err := expectedReliabilityRevision(r); if err != nil { writeError(w, http.StatusBadRequest, "EXPECTED_REVISION_REQUIRED", "a positive If-Match revision is required"); return }
	updated, err := store.TransitionIncident(r.Context(), incident.ID, expected, action, actor, summary); if err != nil { writeStoreError(w, err); return }
