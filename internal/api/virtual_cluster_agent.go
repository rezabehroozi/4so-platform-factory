...
type virtualClusterDispatchInput struct {
	TaskFenceToken int64  `json:"taskFenceToken"`
	Action         string `json:"action"`
}

func (s *Server) dispatchVirtualClusterTask(w http.ResponseWriter, r *http.Request) {
	agentDigest, err := s.agentCredentialDigest(r, r.PathValue("id"))
	if err != nil {
		writeError(w, http.StatusUnauthorized, "AGENT_TOKEN_REQUIRED", err.Error())
		return
	...
	var in virtualClusterDispatchInput
	if err = decodeJSON(w, r, &in); err != nil {
		writeError(w, http.StatusBadRequest, "INVALID_REQUEST", err.Error())
		return
	}
	task, replay, err := s.store.DispatchVirtualClusterTask(
		r.Context(), r.PathValue("id"), agentDigest, r.PathValue("virtualClusterId"),
		rev, in.TaskFenceToken, in.Action,
	)
	if err != nil {
		writeStoreError(w, err)
		return
	...
	setRevisionETag(w, task.ClusterRevision)
	writeJSON(w, http.StatusOK, map[string]any{"task": task, "idempotentReplay": replay})
}
