...
func New(config Config) (*Runtime, error) {
	...
	return &Runtime{config: config, client: &http.Client{Timeout: config.Timeout}}, nil
}
...
func (r *Runtime) Policy() map[string]any {
	if r == nil {
		return map[string]any{"runtimeAuthority": RuntimeAuthority, "enabled": false, "provider": ProviderNone, "redactionRequired": true, "structuredOutputRequired": true, "rawPromptPersisted": false, "advisoryOnly": true, "canDecidePass": false, "canDecidePhysicalPass": false}
	}
	return map[string]any{"runtimeAuthority": RuntimeAuthority, "enabled": r.Enabled(), "provider": r.Provider(), "model": r.Model(), "timeoutSeconds": int(r.config.Timeout / time.Second), "maxInputBytes": r.config.MaxInputBytes, "maxOutputTokens": r.config.MaxOutputTokens, "redactionRequired": true, "structuredOutputRequired": true, "rawPromptPersisted": false, "advisoryOnly": true, "canDecidePass": false, "canDecidePhysicalPass": false}
}
