...
func clusterPeerProbeCommand(clusterAddress, clusterInterface, primaryClusterAddress string) string {
	...
	if clusterInterface != "" {
		...
		probe = "; ip -4 -o addr show | awk '{print $4}' | cut -d/ -f1 | grep -Fxq " + haShellQuote(clusterAddress) + " || { echo " + haShellQuote("cluster address "+clusterAddress+" is not assigned") + " >&2; exit 16; }"
	}
	if primaryClusterAddress != "" && primaryClusterAddress != clusterAddress {
		probe += "; ip -4 route get " + haShellQuote(primaryClusterAddress) + " >/dev/null 2>&1 || { echo " + haShellQuote("no route to primary cluster address "+primaryClusterAddress) + " >&2; exit 17; }"
		if clusterInterface != "" {
			pattern := "(^| )dev " + clusterInterface + "( |$)"
			probe += "; ip -4 route get " + haShellQuote(primaryClusterAddress) + " | grep -Eq " + haShellQuote(pattern) + " || { echo " + haShellQuote("route to primary cluster address "+primaryClusterAddress+" does not use "+clusterInterface) + " >&2; exit 17; }"
		}
	}
	return probe
}
